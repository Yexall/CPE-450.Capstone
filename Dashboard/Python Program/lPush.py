...
# You run the program using the command: python ./lPush.py
#
...
def pushMetric(metricArray):
    if metricArray:  # Check if there is any metric to push.
        ...
        if response.status_code == 200:
            print(f"Successfully pushed {len(metricArray)} metricArray to Prometheus.")
        else:
...
def fetchData():
    # Parse the XML file "UMC500-Curr_109.xml" and get the root element.
    tree = ET.parse("UMC500-Curr_114.xml")  # Change the file name if needed.
    root = tree.getroot()
                ...
                if componentName == "Spindles":
                    ...
                    if events is not None:
                        ...
                        for message in events.findall("m:Message", namespace):
                            # Retrieve the metric's name, unique identifier, and its value from the XML.
                ...
                if componentName == "MachineController":
                    ...
                    if events is not None:
                        ...
                        for message in events.findall("m:Message", namespace):
                            ...
                            # Check if the message is for ActiveAlarms.
                            if name == "ActiveAlarms":
                                dataType = "AlarmStatus"  # Set the data type for alarm status.
                                # Format and push the ActiveAlarms metric.
                                formatMetric(metricArray, deviceName, name, itemID, value, dataType)
